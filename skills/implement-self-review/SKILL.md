...
10. Implement all other pending requested changes.
   - **Deferring is a high bar, not an escape hatch.** Defer an item only when it genuinely cannot be settled by a code change in an unattended pass: a contested repo-convention interpretation, a release-owner / minimum-version / rollout decision, or an open design question with no agreed answer. Anything with a concrete, uncontested fix must be implemented now — do not defer it just because it is large or touches sensitive code.
   - For every item you do defer, record the item and the specific reason it needs a human. You will surface these in step 11.

11. Commit and push the changes — do NOT leave this to the user:
   - **If you implemented nothing this round because every pending Blocking/Important item was deferred in step 10** (`git status --porcelain` is empty): do not create an empty commit, do not push. Instead, end your final response with an explicit, structured "implemented nothing — needs a human" outcome: the literal line `SELF-REVIEW: nothing implemented — <N> item(s) need a human decision`, then one bullet per deferred item (`<item> — <specific reason it needs a human>`). This is what `self-review-pr` turns into a single short PR comment, so make it quotable. Then go to step 12.
   - Stage exactly the files you modified using `git add <file1> <file2> ...` — never `git add -A` or `git add .`.
   - Write the commit message to `/private/tmp/self-review-commit-msg.txt` using the Write tool (not Bash). First line must start with `Self-review:`, followed by a one-bullet-per-item summary of what was changed and why, and any items intentionally skipped and the reason. Never add a `Co-Authored-By` line, a "Generated with Claude Code" line, or any other assistant-attribution trailer — this repo's required CI gate scans every commit message in a PR and blocks merge on one.
...
12. Auto-continue the self-review chain, capped so it can't loop forever:
   - **Chain-completion marker.** On the two exit paths in this step where you stop *after having pushed a commit this round but without opening a next-round terminal* — the 6-round cap, and the terminal-bridge-unreachable case — run `touch /tmp/.claude-self-review-chain-done-<owner>-<repo>-<number>` (replace any `/` in owner/repo with `-`) as your final file operation. This is the deterministic, PR-scoped signal a parent skill (`implement-multiple-reviews`) polls for to learn the chain has terminated within seconds. Do **not** write it when you successfully open a next-round terminal (that round writes it when it terminates), and do **not** write it in the deferred/no-push case below — there, `self-review-pr` writes it *after* posting its "needs a human" comment, so a parent that sees the marker can always find that comment already on the PR.
   - **If step 11 pushed nothing because every item was deferred**, stop here: do NOT open a next-round terminal (an identical unattended pass would hit the same wall), do NOT create or increment the counter file, and do NOT write the chain-completion marker (`self-review-pr` does that after it posts the comment). The "nothing implemented — needs a human" summary from step 11 is this chain's result; `self-review-pr` will post it to the PR.
   ...
   - `MAX_AUTO_ROUNDS` is **6**. If the incremented count is greater than 6, stop here — do not open a terminal; `touch` the chain-completion marker. Say so plainly in your final response (e.g. "Stopped the automatic self-review chain after 6 rounds — this PR may still need a manual look.").
   - Otherwise, open a fresh terminal via the Claude Terminal Bridge extension using four simple, statically-analyzable steps. Do NOT combine these into one multi-line Bash script with `$(...)` command substitution and an `if` guard — that shape gets flagged as "cannot be statically analyzed" and forces a manual permission prompt on every single run, no matter what's in `allowed-tools` or `settings.json`. A flat, literal command is what actually auto-approves:
     ...
     3. Use the `Write` tool to write the request body as literal JSON to `/private/tmp/claude-terminal-bridge-body.json`. Substitute `<N>` with the exact incremented value you just wrote to the counter file — e.g. if the counter file now contains `3`, write `Self-review round 3`, not `2`, not `round 1+1`, the literal number from the file. Substitute the PR URL directly (plain JSON-string escaping only, e.g. escaping the `"` around `Self review this pr: ...`). Set `callerPids` to the JSON array from step 2. Also set `cwd` to the absolute path of the current project root (the working directory this skill is running in) as a fallback the bridge uses only if no window's terminal PIDs match `callerPids`:
        ```json
        {"name": "Self-review round <N>", "cmd": "claude --permission-mode auto \"Self review this pr: <PR_URL>\"", "cwd": "<PROJECT_ROOT_ABSOLUTE_PATH>", "callerPids": [<PID_LIST_FROM_STEP_2>]}
        ```
        `--permission-mode auto` is required, not optional — this terminal has nobody present to answer a permission prompt. A bare `claude "..."` starts in manual mode, and the round hangs forever the moment it hits its first Bash/Edit call needing approval (e.g. clearing a stale marker file), which silently strands the whole chain: the parent skill's wait loop can only tell you it timed out, not that a prompt is sitting unanswered in a window it can't see.
...
- ALWAYS commit and push when you implemented something — never leave that to the user. Never force-push. The one exception is when every pending Blocking/Important item was legitimately deferred as needing a human decision (step 10's high bar) and there is nothing to commit — then push nothing and emit the structured "nothing implemented — needs a human" summary (step 11) instead.
- Deferring is a high bar. Only items that genuinely can't be settled by code in an unattended pass (contested convention, release-owner/versioning decision, unresolved design question) may be deferred. Do not defer an item just because it is large, sensitive, or would take another round — implement it.
- Do NOT post any comment or review to the PR. All output from this skill is the commit(s) pushed to the branch, or — in the deferred case — the "nothing implemented — needs a human" summary that `self-review-pr` (not this skill) posts as one short comment. This skill never writes to GitHub directly.
- ALWAYS attempt step 12 (auto-continue) after a successful push, respecting the round cap. Never raise the cap or skip the counter file as a shortcut — the cap exists specifically to bound an otherwise-unattended loop of automatic `git push`es. If you pushed nothing because everything was deferred, do not open a next-round terminal and do not touch the counter — the chain ends there.
