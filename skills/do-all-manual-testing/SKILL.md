...
Setup note — bring the environment up in this exact order, so the running app is actually serving the PR's branch before any screenshot is taken:

1. **Check out the PR branch** — `gh pr checkout <pr-number>` (from the repo root). If the working tree has unrelated uncommitted changes, stash or set them aside first; don't switch branches on top of a dirty tree.
2. **Start the Convex watcher** — from `packages/convex`, run `bun run dev` in the background (this is `convex dev`, pointed at the shared dev deployment `dev:colorless-clam-56`). It auto-pushes the branch's backend code on every save and keeps `packages/convex/_generated/*` regenerated. Run **exactly one** — `ps aux | grep "convex dev"` first and kill any stale watcher, and never also fire a one-shot `bunx convex dev --once` while it runs (the two race and one silently reverts the other). Wait for its first "Convex functions ready" line before moving on.
3. **Start Metro** — from `apps/mobile`, run `bun expo start` in the background. Wait for the "Waiting on http://localhost:8081" / QR-code line.
4. **Boot exactly the iPhone 16e simulator** — `xcrun simctl boot "iPhone 16e"` then `open -a Simulator` (or the `agent-device` equivalent). Do **not** use whatever simulator happens to already be booted. Confirm with `xcrun simctl list devices booted` that **iPhone 16e is the only** booted device; shut down any other booted simulator first so `agent-device` and Metro both target the right one.
5. **Launch the app on it** (via `ios-simulator` / `agent-device`) and, before trusting any screenshot, confirm the installed build is pointed at `dev:colorless-clam-56` and Metro shows the device connected.
