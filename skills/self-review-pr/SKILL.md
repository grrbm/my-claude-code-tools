...
- Do NOT post the review content itself to the PR. Self-reviews stay local — this is intentional, since posting the full review as a PR comment on every round was bloating the PR. The only exceptions are the two short status comments below (the "nothing blocking or important left" case, and the "everything was deferred — needs a human" case) — neither of which includes the review body.

**Chain-completion marker.** The point that actually *ends* the chain (no further round scheduled) writes, as its very last file operation, a deterministic PR-scoped marker: `touch /tmp/.claude-self-review-chain-done-<owner>-<repo>-<number>` (replace any `/` in owner/repo with `-`). A parent skill that spawned this chain in a detached terminal (`implement-multiple-reviews`) polls for this marker to learn the chain has reached a terminal state within seconds — it must not infer completion from the round-counter file, which a clean first pass never creates. Exactly one place writes it per chain, and only when **no next round was spawned**: the clean branch below writes it; the deferred branch below writes it (after posting its comment); `implement-self-review` writes it for the 6-round cap and the terminal-bridge-unreachable cases; when a next round *is* spawned, that round writes it when it terminates. Never write it before `implement-self-review` returns.
...
- **If the review has no `### 🚨 Blocking` section and no `### ⚠️ Important` section** (nits/blind spots/open questions don't count): this is the exception case. Post a single short comment and stop here — do NOT invoke `implement-self-review`:
  ...
  - `touch /tmp/.claude-self-review-chain-done-<owner>-<repo>-<number>` — the chain-completion marker described above; the chain ends clean here with no round spawned.
...
- **Otherwise** (at least one Blocking or Important item exists): do not post the review to the PR. Immediately invoke the `implement-self-review` skill with the same PR URL (`$ARGUMENTS`) as the argument. Do not ask the user for confirmation — proceed directly into implementation.
  - **When `implement-self-review` finishes, branch on what it actually did:**
    - **It committed and pushed fixes** → post nothing here. The pushed commit(s) are the signal, and `implement-self-review`'s own step 12 handles the next round (or stops at the 6-round cap). Go straight to the done-marker below.
    - **It implemented nothing because every Blocking/Important item was deferred as genuinely needing a human decision** (contested repo-convention call, release-owner/versioning decision, unresolved design question — `implement-self-review` says so explicitly in its summary): the run must not end silently. Post exactly one short comment — the deferred items and their reasons, NOT the review body:
      - `gh pr comment <number> -R <owner>/<repo> --body "Self-review ran and implemented nothing this round — it found N blocking/important item(s) that need a decision from you first: <item 1> — <why it needs a human>; <item 2> — <why>. No changes pushed; the auto-continue chain is stopped here (another identical pass would hit the same wall)."` — fill in the real items/reasons from `implement-self-review`'s summary, keep it to one or two sentences.
      - Clear the auto-continue round counter: `rm -f /tmp/.claude-self-review-rounds-<owner>-<repo>-<number>` (replace any `/` in owner/repo with `-`). The chain has ended.
      - Then `touch /tmp/.claude-self-review-chain-done-<owner>-<repo>-<number>` — the chain-completion marker. It must be written here, *after* the comment above is posted, so a parent skill that sees the marker can always find that comment on the PR. (`implement-self-review` deliberately does not write the marker in this deferred case; it does write it for the 6-round cap and terminal-bridge-unreachable cases, which reach the "committed and pushed fixes" branch above instead.)
  - After that branch, run as the very last command: `touch "/tmp/.claude-review-done.${ITERM_SESSION_ID##*:}"`. Do NOT run this before implement-self-review finishes.
